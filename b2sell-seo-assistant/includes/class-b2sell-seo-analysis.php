...
class B2Sell_SEO_Analysis {
    ...
    private function render_content_tab( $low_score, $order ) {
        ...
        echo 'var csvButton=document.getElementById("b2sell-export-csv");';
        echo 'if(csvButton){csvButton.addEventListener("click",function(){var rows=document.querySelectorAll("#b2sell-seo-table tbody tr");var csv="Logo,"+b2sellLogo+"\nTítulo,Tipo,Último análisis,Puntaje\n";rows.forEach(function(r){var c=r.querySelectorAll("td");csv+=c[0].innerText+","+c[1].innerText+","+c[2].innerText+","+c[3].innerText+"\n";});csv+="\nDesarrollado por B2Sell SPA";var blob=new Blob([csv],{type:"text/csv;charset=utf-8;"});var link=document.createElement("a");link.href=URL.createObjectURL(blob);link.download="b2sell-seo-report.csv";link.click();});}';
        echo 'var pdfButton=document.getElementById("b2sell-export-pdf");';
        echo 'if(pdfButton){pdfButton.addEventListener("click",function(){if(!window.jspdf||!window.jspdf.jsPDF){return;}const {jsPDF}=window.jspdf;var doc=new jsPDF();var img=new Image();img.src=b2sellLogo;img.onload=function(){doc.addImage(img,"PNG",10,10,40,15);doc.setFontSize(16);doc.text("Reporte de Análisis SEO",10,30);var y=40;doc.setFontSize(10);document.querySelectorAll("#b2sell-seo-table tbody tr").forEach(function(r){var c=r.querySelectorAll("td");doc.text(c[0].innerText+" | "+c[1].innerText+" | "+c[2].innerText+" | "+c[3].innerText,10,y);y+=10;});doc.text("Desarrollado por B2Sell SPA",10,280);doc.save("b2sell-seo-report.pdf");};});}';
        echo 'var historyCanvas=document.getElementById("b2sell-history-chart");';
        echo 'var chart=null;';
        echo 'if(historyCanvas){var ctx=historyCanvas.getContext("2d");chart=new Chart(ctx,{type:"line",data:{labels:[],datasets:[{label:"Puntaje",data:[],borderColor:"#0073aa",fill:false}]},options:{scales:{y:{beginAtZero:true,max:100}}}});}';
        echo 'if(chart){document.querySelectorAll(".b2sell-history-row").forEach(function(row){row.addEventListener("click",function(e){if(e.target.tagName.toLowerCase()==="a"){return;}var data=JSON.parse(this.dataset.history||"[]");if(!data.length){return;}var labels=data.map(function(i){return i.date;});var scores=data.map(function(i){return parseInt(i.score);});chart.data.labels=labels;chart.data.datasets[0].data=scores;chart.update();var historyWrapper=document.getElementById("b2sell-history");if(historyWrapper){historyWrapper.style.display="block";}});});}';
        echo '});';
        echo '</script>';
    }
